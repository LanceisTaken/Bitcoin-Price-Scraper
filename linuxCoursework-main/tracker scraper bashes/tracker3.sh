...
log_file="/home/pi4r/Desktop/linuxCoursework/cron2.log"
exec >> "$log_file" 2>&1
...
# Circulating Supply
CIRCULATING_SUPPLY=$(echo "$MAIN_TAG" | sed -n '2p' | sed 's/<[^>]*>//g' | tr -d 'BTC,')
...
# Total Supply
TOTAL_SUPPLY=$(echo "$MAIN_TAG" | sed -n '3p' | sed 's/<[^>]*>//g' | tr -d 'BTC,')
...
# Max Supply
MAX_SUPPLY=$(echo "$MAIN_TAG" | sed -n '4p' | sed 's/<[^>]*>//g' | tr -d 'BTC,')
...
FULLY_DILUTED_MARKET_CAP=$(echo "$MAIN_TAG" | sed -n '5p' | sed 's/<[^>]*>//g' | tr -d 'BTC,$')
...
echo "$(date +"%Y-%m-%d %H:%M:%S"),$RATIO,$CIRCULATING_SUPPLY,$TOTAL_SUPPLY,$MAX_SUPPLY,$FULLY_DILUTED_MARKET_CAP" >> /home/pi4r/Desktop/linuxCoursework/cryptocurrency_data3.csv
